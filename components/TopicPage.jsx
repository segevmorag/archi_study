                            ...
                            {selectedConcept.images && selectedConcept.images.length > 0 ? (
                                <div className="modal-images-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '16px' }}>
                                    {selectedConcept.images.map((img, idx) => (
                                        <div key={idx} className="modal-image-wrapper" onClick={() => setLightboxImage(assetUrl(img))} style={{ cursor: 'zoom-in' }}>
                                            <img src={assetUrl(img)} alt={`${selectedConcept.title} ${idx + 1}`} className="modal-image" />
                                        </div>
                                    ))}
                                </div>
                            ) : selectedConcept.image && (
                                <div className="modal-image-wrapper" onClick={() => setLightboxImage(assetUrl(selectedConcept.image))} style={{ cursor: 'zoom-in' }}>
                                    <img src={assetUrl(selectedConcept.image)} alt={selectedConcept.title} className="modal-image" />
                                </div>
                            )}
